...
The **shell** is the program that reads your commands and executes them. Learning a few dozen commands lets you navigate any Linux/macOS system, manipulate files, and chain operations together.

## The shell you'll use

- **Bash** — the default on most Linux systems and older macOS. The most widely-documented.
- **Zsh** — the default on modern macOS. Nearly identical to Bash for basic commands.
- **PowerShell** — Windows default. Different syntax, but can do similar things.

This guide focuses on **Bash/Zsh** (Linux/macOS/WSL2). The concepts transfer to PowerShell.
...
**Pipes** (`|`) send output from one command as input to another. **Redirection** (`>`, `>>`) sends output to a file.
...
- [The Linux Command Line](https://linuxcommand.org/tlcl.php) — free comprehensive book
...
- [explainshell.com](https://explainshell.com/) — paste any command, see what each part does
