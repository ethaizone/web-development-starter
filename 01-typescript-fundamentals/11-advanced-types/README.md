...
> **This module is optional.** It covers advanced TypeScript features you'll encounter in real codebases. Come back here when you see types you don't recognize. You don't need this to continue to the Web Dev Track.

## What you'll learn
Write types that adapt, transform, and narrow — making TypeScript work harder for you.

## Key concepts

### Generics — types that take parameters

A generic lets you write a function or type that works with any type, while keeping type safety.
...
`<T>` is a type parameter. You can name it anything, but `T` is convention (short for "Type"). When you call the function, you provide the concrete type.
...
`T extends HasId` means "T can be any type, as long as it has an `id: string` property."
...
A discriminated union uses a shared property (the "discriminant") to distinguish between types.
...
TypeScript narrows the type based on `state.status`. In the `"success"` case, `state.data` is available. In the `"error"` case, `state.message` is available. No type assertions needed.
...
1. A generic function `filterByProperty<T, K>(items: T[], key: K, value: T[K])` that filters an array by a property value. (This is challenging — use the simpler version below if stuck.)

   Simpler alternative: A generic function `findByProperty<T>(items: T[], predicate: (item: T) => boolean): T | undefined` that wraps `find` with proper typing.
...
3. A function `describeOperation(operation: LinkOperation): string` that returns a human-readable description using a switch on `type`.
