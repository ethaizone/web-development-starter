...
Same thing, different syntax. Arrow functions are more common in modern codebases.
...
You rarely need to write `: void` explicitly — TypeScript infers it. But you'll see it in type definitions.
...
The type `() => void` means "a function with no parameters that returns nothing." You'll see this pattern in event handlers later.
...
2. `formatUserName(name, lowercase)` — takes a string and an optional boolean. If `lowercase` is true, return the name in lowercase. Otherwise return it as-is.
3. `createEmail(user, domain)` — takes two strings with a default domain of `"example.com"`. Returns `user@domain`.
