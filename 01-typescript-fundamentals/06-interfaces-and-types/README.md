...
String literal unions (`"active" | "inactive"`) act like enums but are simpler.
...
The `&` (intersection) operator combines types. `Link` has all properties from
`BaseEntity` plus its own.
...
You've already seen this pattern in Module 05. Now you know how to name the type
...
3. `type User` — email, role (`"admin"` | `"member"`), profile (of type
   `Profile`), links (array of `Link`)
4. Create a `User` object with at least 2 links
5. Write a function `getVisibleLinks(user: User): Link[]` that returns only
   links where `isVisible` is true
