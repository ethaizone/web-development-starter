...
Build a complete CLI tool that uses everything from modules 02–11: types,
functions, control flow, arrays, async, error handling, modules, and npm
packages.
...
Node.js includes a built-in `readline` module for reading user input from the
terminal:
...
export function formatProfile(profile: DeveloperProfile): string {
  ...
  if (profile.githubData) {
    ...
    lines.push(
      `    Repos: ${profile.githubData.publicRepos} | Followers: ${profile.githubData.followers}`,
    );
  }
...
async function createNewProfile(): Promise<void> {
  ...
  if (!isValidUsername(username)) {
    console.log(
      "Invalid username. Use 2-30 characters: letters, numbers, _ or -",
    );
    return;
  }
...
async function fetchGitHub(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `GitHub data loaded: ${githubData.publicRepos} repos, ${githubData.followers} followers`,
    );
  } catch (error) {
...
- **Validate on load**: When loading `profiles.json`, validate the data matches
  your types
...
**Congratulations!** You've completed the TypeScript Fundamentals track. You can
now:
...
**Next:**
[Web Development Track](../../02-web-development/01-how-the-web-works/) → Build
a real web application with TanStack Start.
