...
You can throw any value, but always throw `Error` objects — they include a stack
trace.
...
TypeScript types the caught error as `unknown`. Use type narrowing to work with
it.
...
try {
  ...
  if (caughtError instanceof ValidationError) {
    console.log(
      `Validation failed on "${caughtError.field}": ${caughtError.message}`,
    );
  }
}
```
...
1. A function `parseLinkUrl(input: string): { title: string; url: string }`
   that:
   - Throws an `Error` if input is empty
   - Throws a `ValidationError` (custom class) if it doesn't start with
     `http://` or `https://`
   - Returns `{ title: new URL(input).hostname, url: input }` otherwise
2. A function `safeParseLink(input: string)` that calls `parseLinkUrl` in a
   try/catch and returns the result or a user-friendly error string
