...
Make your pages interactive with React state (`useState`), event handlers, and
...
**State** is data that changes over time in your component. Unlike props (passed
in from outside), state is managed **inside** the component.

When state changes, React re-renders the component — the UI automatically
...
React batches state updates for performance. The new value isn't available
immediately:
...
Every keystroke: `onChange` fires → `setEmail` updates state → React re-renders
→ input shows new value.

This pattern gives you full control: validation, formatting, disabling based on
state, etc.
